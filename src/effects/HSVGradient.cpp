...
HSVGradient::HSVGradient(float startPosition, float endPosition, unsigned long runtimeMs, FHSV startGradientStart,
                         FHSV startGradientEnd, FHSV endGradientStart, FHSV endGradientEnd,
...
void HSVGradient::calculate(float startIndex, TemporaryLedData& tempData) {
	...
	if (lo > hi) {
		...
		FHSV tmpColor = loColor; loColor = hiColor; hiColor = tmpColor;
	}
