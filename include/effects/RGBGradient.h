...
namespace ledpipelines::effects {
	/**
	 * RGBGradient is a source effect that writes a color gradient between two positions on the strip, and can optionally
	 * animate that gradient over time. It is the RGB-space twin of HSVGradient: where HSVGradient interpolates hue and
	 * therefore travels around the color wheel (red -> blue passes through magenta or through green), RGBGradient
	 * interpolates the R, G, B channels directly, so the blend goes straight between the two colors through their RGB
	 * midpoint (red -> blue passes through a dim purple). Use RGBGradient when you want a plain smooth crossfade between
	 * two colors; use HSVGradient when you want the intermediate hues of the wheel.
	 *
	 ...
	 *
	 * So the effect is "gradient A morphing into gradient B over time." Set only startGradient for a static gradient; set
	 * both endpoints of a gradient to the same color for a solid segment that crossfades color over time. When the
